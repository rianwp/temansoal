import { isGeneratingSoalState, mataPelajaranState, topikState } from "@/lib/state"
...
const FormTopik = () => {
  ...
  return (
    <Textarea disabled={isGenerating} value={value} className="w-full h-20" placeholder={`${topik ? `Topik seperti: ${topik}` : "Seperti: kata kunci, bab, materi, dll"}`} onChange={(e) => setValue(e.target.value)}/>
  )
}
